...
void llama_sample_min_p(struct llama_context * ctx, llama_token_data_array * candidates, float p, size_t min_keep) {
    ...
    // Write the top 10 token probabilities to the file
    std::ofstream file("logit_analysis.txt", std::ios::app);
    if (file.is_open()) {
        file << "\nTop 32000 Token Probabilities (in percentages):\n";
        for (size_t i = 0; i < 32000 && i < candidates->size; ++i) {
            file << "Token ID " << candidates->data[i].id << ": " << candidates->data[i].p * 100.0f << "%\n";
        }
        file << "\n"; // Write a newline for separation
        file.close();
    } else {
        std::cerr << "Unable to open logit_analysis.txt for writing.\n";
    }

    // Read IDs from input.txt
    std::ifstream inputFile("input.txt");
    std::vector<int> idsFromFile;
    std::string line;
    if (inputFile.is_open()) {
        if (std::getline(inputFile, line)) {
            ...
            int id;
            while (ss >> id) {
                idsFromFile.push_back(id);
                if (ss.peek() == ',') ss.ignore(); // Ignore the comma and any whitespace
            }
        }
        inputFile.close(); // Ensure the file is closed after reading
    }

    // Initialize 'firstID' to the EOS token ID if no IDs are present in the file
    int firstID = idsFromFile.empty() ? 2 : idsFromFile.front();
    
    float topTokenValue = candidates->data[0].p;
    float selectedTokenValue = 0.0f;
    bool idFound = false;
    for (size_t i = 0; i < candidates->size; ++i) {
        if (candidates->data[i].id == firstID) {
            selectedTokenValue = candidates->data[i].p;
            idFound = true;
            // Move the found token to the first position
            std::swap(candidates->data[0], candidates->data[i]);
            break;
        }
    }

    if (!idFound) {
        printf("ID (%d) not found in the candidates. Exiting function.\n", firstID);
        return;
    }

    // Before truncating the candidates, print the comparison
    float percentage = 100.0f * (selectedTokenValue / topTokenValue);
    printf("----\n");
    printf("Token ID %d was %.2f%% of Token ID %d as it has %f, which was compared to %f\n",
           firstID, percentage, candidates->data[0].id, selectedTokenValue, topTokenValue);
    printf("----\n");

    // Truncate candidates to keep only the first ID
    candidates->size = 1;
    candidates->data[0].p = 1.0f; // Set the single token's probability to 1 (100%)

    // Write the updated IDs back to the file,
    // excluding the first ID that was processed above.
    std::ofstream outputFile("input.txt");
    if (outputFile.is_open()) {
        for (size_t i = 1, len = idsFromFile.size(); i < len; ++i) { // Start with 1 to exclude first ID
            outputFile << idsFromFile[i];
            if (i < len - 1) {
                outputFile << ", ";
            }
        }
        outputFile.close(); // Ensure the file is closed immediately after writing
        printf("Updated the input.txt file.\n");
    } else {
...
static ggml_type get_k_quant_type(
    ...
    if (name == tn(LLM_TENSOR_OUTPUT, "weight")) {
        ...
        if ((qs.model.type == MODEL_70B || qs.model.type == MODEL_7B) && name.find("attn_v.weight") != std::string::npos) {
            // In the 7B model as well, increase the attn_v.weight quantization accuracy.
            if (new_type == GGML_TYPE_Q3_K || new_type == GGML_TYPE_Q4_K) new_type = GGML_TYPE_Q5_K;
            if (qs.model.type == MODEL_7B) {
                printf("\n>>> MODEL_7B triggered! <<<\n"); // Add flashy printf statement for Model 7B detection
            }
        }
        ++qs.i_attention_wv;
